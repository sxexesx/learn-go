...
2) Объект слишком большой, то он аллоцируется в куче (> 32Кб)
...
_Пример 2:_ В данном случае функция append создает копию слайса, которая за пределами функции не будет изменена, (но!) сам подлежащий массив будет изменён. см пример в _notes
```golang
func changeSlice(s []int) {
	s = append(d, 4)
}
